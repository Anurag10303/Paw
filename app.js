...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Dashboard ---
  const petProfilesContainer = document.getElementById(
  ...
  const successModalCloseBtn = document.getElementById(
    ...
    pets: [
      {
        ...
        age: 5,
        photo: "https://placehold.co/400x300/FBBF24/FFFFFF?text=Tommy",
      },
      {
        id: 2,
        name: "Pirro",
        ...
        breed: "Persian Cat",
        age: 3,
        photo: "https://placehold.co/400x300/A78BFA/FFFFFF?text=Pirro",
      },
    ],
    appointments: [
      {
  ...
  function renderPetProfiles() {
    ...
    userPets.forEach((pet) => {
      const petCardHTML = `
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 cursor-pointer" data-pet-id="${
                  pet.id
                }">
                    <img src="${
                      pet.photo ||
                      "https://placehold.co/400x300/CBD5E1/475569?text=My+Pet"
                    }" alt="Photo of ${
        pet.name
      }" class="w-full h-48 object-cover" onerror="this.onerror=null;this.src='https://placehold.co/400x300/CBD5E1/475569?text=Image+Error';">
                    <div class="p-6">
  ...
  function renderAppointments() {
    ...
    noAppointmentsPlaceholder.classList.toggle(
      "hidden",
    ...
    userAppointments.forEach((appt) => {
      ...
      const petPhoto = pet
        ? pet.photo
        : "https://placehold.co/100/CBD5E1/475569?text=?";
      ...
      const column = document.querySelector(
        `.appointment-column[data-status="${appt.status}"]`
      );
      if (column) {
  ...
  function renderPetProfilePage(petId) {
    ...
    const profileHTML = `
      ...
      pet.name
    }" class="w-48 h-48 mx-auto rounded-full object-cover shadow-lg border-4 border-purple-200" onerror="this.onerror=null;this.src='https://placehold.co/400x300/CBD5E1/475569?text=Image+Error';">
                    ...
                    <div class="md:w-2/3">
                        ...
                        <div class="mt-8 flex justify-end space-x-4">
                            <button class="edit-btn-profile px-6 py-2 text-white font-semibold bg-blue-600 rounded-lg hover:bg-blue-700 transition-transform transform hover:scale-105" data-pet-id="${
                              pet.id
                            }"><i class="fas fa-edit mr-2"></i>Edit</button>
                            <button class="delete-btn-profile px-6 py-2 text-white font-semibold bg-red-500 rounded-lg hover:bg-red-600 transition-transform transform hover:scale-105" data-pet-id="${
                              pet.id
  ...
  navBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (pageId === "setup-page") {
        ...
        setupFormSubmitBtn.innerHTML =
          '<i class="fas fa-check"></i><span>Create Pet Profile</span>';
      }
      showMainPage(pageId);
    });
  });
  ...
  petProfilePage.addEventListener("click", (event) => {
    ...
    if (event.target.classList.contains("edit-btn-profile")) {
      ...
      if (petToEdit) {
        ...
        photoPreview.innerHTML = petToEdit.photo
          ? `<img src="${petToEdit.photo}" alt="Pet photo" class="w-full h-full object-cover rounded-full">`
          : '<i class="fas fa-plus text-3xl text-gray-400"></i>';
